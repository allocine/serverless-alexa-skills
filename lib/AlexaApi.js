  ...
  getModels(vendorId) {
    return BbPromise.resolve(this.getSkills(vendorId))
      .mapSeries((remote) => {
        ...
        const skillLocales = remote.manifest.publishingInformation.locales;
        const locales = Object.keys(skillLocales)
          .map(locale => ({ id: remote.skillId, locale }));
        return BbPromise.resolve(locales)
          .map(locale => this
            .get(`/skills/${locale.id}/stages/development/interactionModel/locales/${locale.locale}`)
      ...
      .then(res => BbPromise.resolve(res.reduce((acc, arr) => acc.concat(arr))));
  }
